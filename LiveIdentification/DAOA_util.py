...
def get_prediction(img, confidence):
    ...
    if len(pred_t) == 0:
        return None, None, None, None
    masks = (pred[0]['masks'] > confidence).detach().cpu().numpy()
...
def digit_area_orientation_alignment(img, og_img, confidence=0.9, rect_th=2, text_size=2, text_th=2):
    """
    ...
    if masks is None:
        ...
        for i in range(len(pred_cls)):
            ...
            if pred == 'bar':
                ...
                cv2.putText(annImg, pred+": "+str(int(pred_score[i]*100)/100).lstrip('0'), (box[0][0], box[0][1]-10), cv2.FONT_HERSHEY_SIMPLEX,
